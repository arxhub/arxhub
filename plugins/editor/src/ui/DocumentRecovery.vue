...
<template>
  <Dialog open :title="conflict ? 'File changed outside this editor' : 'Recover unsaved draft'" size="lg" :close-on-escape="false" :close-on-interact-outside="false">
    ...
    <template #footer>
      <Button variant="ghost" :disabled="busy" @click="emit('choose', 'saved')">Keep saved file</Button>
      <Button variant="secondary" :disabled="busy" @click="emit('choose', 'both')">Keep both</Button>
      <Button :disabled="busy" @click="emit('choose', 'draft')">{{ conflict ? 'Replace saved file with draft' : 'Recover draft' }}</Button>
    </template>
  </Dialog>
</template>
