...
export function parseMarkdownBlocks(body: string): SourceBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (fence != null) {
      ...
      blocks.push({ type: 'code', level: null, raw: code, content: code })
      continue
    }
    ...
    if (heading != null) {
      ...
      blocks.push({ type: 'heading', level: heading[1].length, raw, content: stripInlineMarkup(raw) })
      i += 1
      continue
    }
    ...
    if (quote != null) {
      ...
      blocks.push({ type: 'quote', level: null, raw, content: stripInlineMarkup(raw) })
      continue
    }
    ...
    if (item != null) {
      ...
      blocks.push({ type: 'list-item', level: null, raw, content: stripInlineMarkup(raw) })
      continue
    }
    ...
    blocks.push({ type: 'paragraph', level: null, raw, content: stripInlineMarkup(raw) })
  }
